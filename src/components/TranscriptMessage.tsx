...
function findToolResult(toolId: string, allMessages: TranscriptMessageType[]): { content: string; isError: boolean } | undefined {
...
function calculateAggregatedStats(toolMessages: TranscriptMessageType[]): ToolStats {
  let totalCalls = 0;
  let totalInput = 0;
  let totalOutput = 0;
  let totalCached = 0;
...
export function TranscriptMessage({
  ...
  const hasAggregatedTools = message.aggregatedToolMessages && message.aggregatedToolMessages.length > 0;
  ...
  const hasOnlyDirectTools = isAssistant && !textContent.trim() && directToolCalls.length > 0 && !hasAggregatedTools;

  return (
    <div className={cn(
      "flex flex-col",
      depth > 0 && !isChronological && "ml-8"
    )}>
      <div className={cn(
        "flex",
        ...
        {textContent && (
          <div className={cn(
            ...
            <MessageContent message={message} allMessages={allMessages} showOnlyText={true} />
          </div>
        )}

        {/* Usage Stats (when no tool calls) */}
        {!hasAggregatedTools && !hasOnlyDirectTools && message.message?.usage && (
          <div className="flex gap-2 text-xs text-muted-foreground">
            ...
            {message.message.usage.cache_read_input_tokens && (
              <span>Cache: {message.message.usage.cache_read_input_tokens}</span>
            )}
          </div>
        )}
        </div>
            ...
            >
              <div className="flex items-center gap-2">
                <span className="flex items-center gap-1">
                  ...
                  {aggregatedStats.totalCalls} tool {aggregatedStats.totalCalls === 1 ? "call" : "calls"}
                </span>
                <span className="text-muted-foreground/50">•</span>
                <span>{aggregatedStats.totalInput.toLocaleString()} input</span>
                <span className="text-muted-foreground/50">•</span>
                <span>{aggregatedStats.totalOutput.toLocaleString()} output</span>
                {aggregatedStats.totalCached > 0 && (
                  <>
                    <span className="text-muted-foreground/50">•</span>
                    <span>{aggregatedStats.totalCached.toLocaleString()} cached tokens</span>
                  </>
                )}
              </div>
