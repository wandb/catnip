...
export class FileDropAddon implements IDisposable {
  ...
  private _onDragLeave(e: DragEvent): void {
    ...
    if (this._terminal?.element && !this._terminal.element.contains(e.relatedTarget as Node)) {
  ...
  private _sendPathAsInput(rawPath: string): void {
    if (!this._sendData) {
      console.warn('No sendData callback provided, cannot send file path as input')
      return
    }
