...
# Extract feature version from devcontainer-feature.json
FEATURE_VERSION=""
if command -v jq >/dev/null 2>&1; then
  FEATURE_VERSION=$(jq -r '.version' "$(dirname "$0")/devcontainer-feature.json" 2>/dev/null || echo "unknown")
else
  # Fallback without jq - extract version using grep/sed
  FEATURE_VERSION=$(grep '"version"' "$(dirname "$0")/devcontainer-feature.json" 2>/dev/null | sed -E 's/.*"version"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/' || echo "unknown")
fi
...
EOF
  ...
  # Install init service for catnip auto-start
  install_init_service

}

install_init_service() {
  log "Installing init service for catnip auto-start..."

  # Copy init script to /etc/init.d/
  local init_script="/etc/init.d/catnip"
  ...
  run_as_root tee /etc/default/catnip >/dev/null <<EOF
# Catnip service environment variables
# Generated by catnip devcontainer feature v$FEATURE_VERSION
...
EOF
  ...
  # Install the service using update-rc.d if available, or just enable manually
  if command -v update-rc.d >/dev/null 2>&1; then
    run_as_root update-rc.d catnip defaults
  else
    log "update-rc.d not available, service will need to be started manually"
  fi

  ok "Init service installed"
  log "Manual control: service catnip {start|stop|restart|status}"
  log "Or use: /etc/init.d/catnip {start|stop|restart|status}"
}
...
main() {
  shopt -s nocasematch
  log "Catnip devcontainer feature v$FEATURE_VERSION"
  debug_feature "Feature version: $FEATURE_VERSION"
  debug_feature "Debug mode: $DEBUG"
  debug_feature "Dev mode: $CATNIP_DEV"
