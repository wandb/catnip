...
func startServer(cmd *cobra.Command) {
	...
	if config.Runtime.IsNative() {
		logger.Debugf("📁 Workspace: %s", config.Runtime.WorkspaceDir)
		logger.Debugf("⚙️  Config: %s", config.Runtime.VolumeDir)
		if config.Runtime.CurrentRepo != "" {
			logger.Debugf("📦 Current repo: %s", config.Runtime.CurrentRepo)
		}
	}
	...
	if catnipRepo := os.Getenv("CATNIP_REPO"); catnipRepo != "" {
		logger.Infof("🌟 CATNIP_REPO detected: %s", catnipRepo)
		go func() {
			...
			if len(repoParts) >= 2 {
				...
				logger.Infof("📦 Auto-checkout: %s/%s (branch: %s)", org, repo, branch)
				if _, _, err := gitService.CheckoutRepository(org, repo, branch); err != nil {
					logger.Errorf("❌ Failed to checkout repository: %v", err)
				}
			}
		}()
	}
	...
	logger.Debugf("✅ setupExecutor configured successfully")
	...
	logger.Debugf("🔄 Restoring worktree state from persistent storage")
	...
	if err := claudeMonitor.Start(); err != nil {
		...
		logger.Debugf("✅ Claude monitor service started successfully")
	}
	...
	logger.Debugf("✅ EventsHandler connected to GitService for worktree cache events")
	...
	logger.Debugf("✅ ClaudeMonitorService connected to SessionService for real-time activity tracking")
	...
	if handlers.IsDevMode() {
		...
		logger.Infof("🔧 Development mode: proxying to Vite dev server %s", handlers.ViteServerURL())
		...
		if handlers.HasEmbeddedAssets() {
			logger.Infof("🚀 Production mode: serving embedded frontend assets")
			...
			logger.Infof("⚠️  Production mode: frontend assets not embedded, serving from ./dist")
	...
	logger.WithField("port", port).Fatal().Msg("Server failed to start")
	if err := app.Listen(":" + port); err != nil {
		logger.WithField("error", err).Fatal().Msg("Server failed to start")
	}
}
